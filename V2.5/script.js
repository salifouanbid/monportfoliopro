document.addEventListener('DOMContentLoaded', () => {
    ...
    function afficherImagesAvecDelai() {
            // Sélectionnez tous les éléments <img> dans le document
    const images = document.querySelectorAll('img');

            // Parcourez chaque image
        images.forEach((img, index) => {
                // Définissez un délai pour afficher l'image
            setTimeout(() => {
                    // Ajoutez l'image au conteneur
                document.getElementById('image-container').appendChild(img);
            }, index * 1000); // Chaque image attend 1000ms (1 seconde) avant d'être affichée
        });
    }
    afficherImagesAvecDelai();
